...
export async function googleTranslate(content: string, options: GoogleTranslateOptions) {
  ...
  // 谷歌翻译使用系统代理
  const fetchOptions: Partial<RequestInit> | undefined = (useSystemProxy && HTTP_PROXY)
    ? {
        agent: createHttpProxyAgent({
          host: new URL(HTTP_PROXY).hostname,
          port: new URL(HTTP_PROXY).port,
          secureProxy: Boolean(HTTPS_PROXY),
        }),
      } as RequestInit
    : undefined

  return new Promise<string>((resolve, reject) => {
    google(content, { from: sourceLang, to: targetLang, fetchOptions }).then(res => resolve(res.text)).catch(reject)
  })
}
...
export async function translate(content: string, options: TranslateOptions) {
